...
public class PrinterService implements RegisterListener, Runnable {
	...
	// Do the registration
	public PrinterService() throws DNSSDException, InterruptedException {

		// Load the description file
		InputStream in = PrinterService.class.getResourceAsStream("description.properties");
		p = new Properties();
		if (in != null) {
			try {
	...
	public void serviceRegistered(DNSSDRegistration registration, int flags, String serviceName, String regType,
		...
		System.out.println( " register complete in " + System.currentTimeMillis());
	}
	...
	private class SimulateService implements Runnable {
		...
		public void run() {
			...
			try {
				...
				System.out.println(serviceName+" has conneted with controller : "+requestName);
